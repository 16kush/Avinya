...
class TeacherPage extends Component {
	state = {
		Form: {
			...
			descriptionLong: {
				...
				cols: "50",
				placeholder:
					"Entereg: In this course you will learn how to build professional website from scratch and how to make it responsive. Course Title",
				value: "",
				valid: true,
	...
	CKEditorHandler = (event, editor, Title) => {
		...
		updatedForm[Title].valid = this.checkValidity(
			data,
			updatedForm[Title].validation
		);
	...
	sumbitButton = () => {
		...
		} else this.AlertError("Validation Errors!", "warning");
	};
	...
	render() {
		...
		if (this.state.Form.category["Development"]) {
			classWeb = ["ButtonClicked"];
			//console.log("clicked11",classWeb.join(' '),this.state.Form.category['Development'])
		} else classWeb = [];

		if (this.state.Form.category["Designing"]) {
				...
				<div className='container-fluid-main'>
					...
					<div className='Teacher-Courses-Buttons-head'>
						...
						<div className='Teacher-Courses-Buttons'>
							<button
								onClick={() => this.categoryHandler("Development")}
								className={classWeb.join(" ")}>
								{" "}
								Development
							</button>
							<button
								className={classWebDesign.join(" ")}
								onClick={() => this.categoryHandler("Designing")}>
								{" "}
								Designing
							</button>
							<button
								className={classReact.join(" ")}
								onClick={() => this.categoryHandler("React")}>
								{" "}
								React
							</button>
							<button
								className={classML.join(" ")}
								onClick={() => this.categoryHandler("ML")}>
								{" "}
								ML
							</button>
							<button
								className={classPhotography.join(" ")}
								onClick={() => this.categoryHandler("Photography")}>
								{" "}
								Photography
							</button>
							<button
								className={classNodeJs.join(" ")}
								onClick={() => this.categoryHandler("NodeJs")}>
								{" "}
								Node JS
							</button>
						</div>
					...
					<div className='Teacher-Head-Class'>
						<CKEditorArea
							label={this.state.Form.descriptionLong.label}
							rows={this.state.Form.descriptionLong.rows}
							cols={this.state.Form.descriptionLong.cols}
							placeholder={this.state.Form.descriptionLong.placeholder}
							changed={(event, editor) =>
								this.CKEditorHandler(event, editor, "descriptionLong")
							}

							// changed={(event)=> this.inputchangeHandler(event,"descriptionLong")}
						/>
					</div>
					...
					<div className='Teacher-Head-Class'>
						<CKEditorArea
							...
							changed={(event, editor) =>
								this.CKEditorHandler(event, editor, "willLearn")
							}
						/>
					</div>
					...
					<div className='Teacher-Head-Class ckeditor'>
						<CKEditorArea
							...
							changed={(event, editor) =>
								this.CKEditorHandler(event, editor, "requirement")
							}
						/>
					</div>
					...
					<div className='Teacher-Head-Class'>
						...
						<img
							className=''
							src={this.state.Form.image.name}
